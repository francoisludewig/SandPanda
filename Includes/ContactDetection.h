...
public:
	...
	static void ContactSphPlanPeriodic(Sphere *llist[], Plan & p, Plan & p2, Sphere *b, const double rmax) noexcept;
	...
	static void sphContactAll(const int & Nsph, std::vector<Sphere> & sph, Contact *ctl, int & Nctl, double dila) noexcept;
	static void sphContactAll(const int & Nsph, std::vector<Sphere> & sph, Contact *ctl, int & Nctl) noexcept;

	static void sphPlanContact(const int &Nsph, const int &Npl, int & Nct, std::vector<Sphere> & sph,
	...
	static void sphPlanRContact(const int &Nplr, int & Nct, std::vector<PlanR> & plr, Contact *ct, Sphere *cell[]) noexcept;
	
	static void sphConeContact(const int &Nco, int & Nct, std::vector<Cone> & co, Contact *ct, Sphere *cell[]) noexcept;
	
	static void sphElbowContact(const int &Nelb, int & Nct, std::vector<Elbow> & elb, Contact *ct, Sphere *cell[]) noexcept;
	
	static void sphHollowBallContact(const int &Nhb, int & Nct, std::vector<HollowBall> & hb, Contact *ct) noexcept;
	
	static void sphContainer(const int &Npsh, const int & Npl, const int & Nplr, const int & Nco, const int & Nelb,
													 std::vector<Sphere> & sph, std::vector<Plan> & pl, std::vector<PlanR> & plr,
													 std::vector<Cone> & co, std::vector<Elbow> & elb, int & Nct, Contact *ct, Sphere *cell[],
													 const double rmax) noexcept;
	static void sphContainer(const int & Nsph, const int & Npl, const int & Nplr, const int & Nco, const int & Nelb,
													 const int & Nhb, std::vector<Sphere> & sph, std::vector<Plan> & pl, std::vector<PlanR> & plr, std::vector<Cone> & co, std::vector<Elbow> & elb, std::vector<HollowBall> & hb, int & Nct, Contact *ct, Sphere *cell[], const double rmax) noexcept;
