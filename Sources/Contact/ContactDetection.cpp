    ...
    if (a->Num() != b->Num()) {
        if ((a->Bodies() == -9 && b->Bodies() == -9)) {
            ...
            if (a->Bodies() != -9 && b->Bodies() != -9) {
                ...
                rtn = ContactDetectorBodyVersusBody::Detect(a->GetBody(), b->GetBody(), ct, Nct, a->Radius(),
                                                            b->Radius());
                (a->GetBody())->X(tpx);
                (a->GetBody())->Y(tpy);
                (a->GetBody())->Z(tpz);
...
void ContactDetection::ContactSphPlanPeriodic(std::vector<Sphere *> &llist, Plan &p, Plan &p2, Sphere *b,
...
void ContactDetection::sphContact(const CellBounds &cellBounds, Contact *ctl, int &Nctl,
    ...
    const int startx = cellBounds.StartX();
    const int starty = cellBounds.StartY();
    const int startz = cellBounds.StartZ();
    const int endx = cellBounds.EndX();
    const int endy = cellBounds.EndY();
    const int endz = cellBounds.EndZ();
    const int maxx = cellBounds.MaxX();
    const int maxy = cellBounds.MaxY();
    const int maxz = cellBounds.MaxZ();
...
#pragma omp parallel
    {
        ...
        int num;
        int localNct = 0;
        const int maxLocalContacts = 5000;
        Contact localCt[maxLocalContacts];
...
#pragma omp for schedule(dynamic)
            ...
            for (int j = starty; j < endy; j++) {
                for (int k = startz; k < endz; k++) {
                    ...
                    if ((cand[Ncand] = cell[num]) != nullptr) {
                        ...
                        for (l = 0; l < Ncand; l++) {
                            for (m = l + 1; m < Ncand; m++)
                                ContactDetectorSphereVersusSphere::Detect(cand[l], cand[m], localCt, localNct);
                        }
                        if (i + 1 < maxx) {
                            if ((anta = cell[num + maxy * maxz]) != nullptr) {
                                do {
                                    for (l = 0; l < Ncand; l++)
                                        ContactDetectorSphereVersusSphere::Detect(cand[l], anta, localCt, localNct);
        ...
        // Merge thread-local contacts into global array (thread-safe)
        int offset;
#pragma omp critical
        {
            offset = Nctl;
        ...
        std::memcpy(&ctl[offset], localCt, localNct * sizeof(Contact));
    }
#else
    // Sequential fallback
    int l, m;
...
void ContactDetection::sphPlanContact(int &Nct, std::vector<Sphere> &sph, std::vector<Plan> &pl, Contact *ct,
                                      std::vector<Sphere *> &cell, const SolidCells &solidCells,
                                      const double rmax) noexcept {
    Sphere *anta;
...
void ContactDetection::sphPlanRContact(int &Nct, std::vector<PlanR> &plr, Contact *ct, std::vector<Sphere *> &cell,
                                       const SolidCells &solidCells) noexcept {
    Sphere *anta;
    for (auto &disk: plr) {
...
void ContactDetection::sphConeContact(int &Nct, std::vector<Cone> &co, Contact *ct, std::vector<Sphere *> &cell,
                                      const SolidCells &solidCells) noexcept {
    Sphere *anta;
    for (auto &cone: co) {
...
void ContactDetection::sphElbowContact(int &Nct, std::vector<Elbow> &elb, Contact *ct, std::vector<Sphere *> &cell,
                                       const SolidCells &solidCells) noexcept {
    Sphere *anta;
    for (auto &elbow: elb) {
...
void ContactDetection::sphContainer(std::vector<Sphere> &sph, std::vector<Plan> &pl, std::vector<PlanR> &plr,
                                    std::vector<Cone> &co, std::vector<Elbow> &elb, std::vector<HollowBall> &hb,
                                    int &Nct, Contact *ct, std::vector<Sphere *> &cell, const SolidCells &solidCells,
                                    const double rmax) noexcept {
