...
/* Fonction qui calcul les forces pour l'ensemble des contacts du tableau ct */
void ComputeForce::ComputeMutex(Contact *ct, const int Nct, Configuration & dat) noexcept {
	double lax,lay,laz,lbx,lby,lbz;
	double Vax,Vay,Vaz,Vbx,Vby,Vbz;
	double wbx,wby,wbz;
	double Vn,Vtx,Vty,Vtz,Vt;
		...
		switch(ctl->type){
			...
			case Contact::Type::SphereSphere: //0
				...
				if(ctl->delta <= 0){
					...
					// Statique
						...
						//printf("Add Xsi = %e\t%e\t%e\n\n", xsi.x, xsi.y, xsi.z);
						a->GetElongationManager().AddElongationMutex(xsi,b->Num(),Contact::Type::SphereSphere,-9);
						b->GetElongationManager().AddElongationMutex(xsi,a->Num(),Contact::Type::SphereSphere,-9);
					}
				}
			...
			case Contact::Type::SphereBody: //10
				...
				if(ctl->delta <= 0){
					...
					// Statique
						...
						computeContactForceStatic(ctl, xsi, N, T, tx, ty, tz, Vn, Vt, Vtx, Vty, Vtz, h, k, g0, dat.muS,dat.muD);
						// Enregistrement du nouveau Xsi
						a->GetElongationManager().AddElongationMutex(xsi,bb->Num(),Contact::Type::SphereBody,nb);
						bb->GetElongationManager().AddElongationMutex(xsi,a->Num(),Contact::Type::SphereBody,nb,-9);
					}
				}
			...
			case Contact::Type::BodyBody: //20
				...
				if(ctl->delta <= 0){
					...
					// Statique
						...
						xsi = ba->GetElongationManager().GetElongation(bb->Num(),Contact::Type::BodyBody,na,nb);
						computeContactForceStatic(ctl, xsi, N, T, tx, ty, tz, Vn, Vt, Vtx, Vty, Vtz, h, k, g0, dat.muS,dat.muD);
						ba->GetElongationManager().AddElongationMutex(xsi,bb->Num(),Contact::Type::BodyBody,na,nb);
						bb->GetElongationManager().AddElongationMutex(xsi,ba->Num(),Contact::Type::BodyBody,nb,na);
					}
				}

				ComputeContactForce(ctl, tx, ty, tz, T, N, Fx, Fy, Fz);

				ba->AddForceMutex(Fx, Fy, Fz);
				ba->AddMomemtumMutex((lay*Fz-laz*Fy), (laz*Fx-lax*Fz), (lax*Fy-lay*Fx));
			...
			case Contact::Type::SpherePlan: //1
				...
				if(ctl->delta <= 0){
					...
					// Statique
						...
						xsi = a->GetElongationManager().GetElongation(p->Numero(),Contact::Type::SpherePlan,-9);
						computeContactForceStatic(ctl, xsi, N, T, tx, ty, tz, Vn, Vt, Vtx, Vty, Vtz, h, k, g0, dat.muS,dat.muD);
						a->GetElongationManager().AddElongationMutex(xsi,p->Numero(),Contact::Type::SpherePlan,-9);
					}
				}
				ComputeContactForce(ctl, tx, ty, tz, T, N, Fx, Fy, Fz);
                /*
			    ...
			    p->AddForceMutex(Fx, Fy, Fz);
			    p->AddMomemtumMutex((lby*Fz-lbz*Fy), (lbz*Fx-lbx*Fz), (lbx*Fy-lby*Fx));
			...
			case Contact::Type::BodyPlan: //11
				...
				if(ctl->delta <= 0){
					...
					// Statique
						...
						xsi = ba->GetElongationManager().GetElongation(p->Numero(),Contact::Type::BodyPlan,na,-9);
						computeContactForceStatic(ctl, xsi, N, T, tx, ty, tz, Vn, Vt, Vtx, Vty, Vtz, h, k, g0, dat.muS,dat.muD);
						ba->GetElongationManager().AddElongationMutex(xsi,p->Numero(),Contact::Type::BodyPlan,na,-9);
					}
				}
			...
			case Contact::Type::SpherePlanR: //2
				...
				if(ctl->delta < 0){
					...
					// Statique
						...
						xsi = a->GetElongationManager().GetElongation(pr->Numero(),Contact::Type::SpherePlanR,-9);
						computeContactForceStatic(ctl, xsi, N, T, tx, ty, tz, Vn, Vt, Vtx, Vty, Vtz, h, k, g0, dat.muS,dat.muD);
						a->GetElongationManager().AddElongationMutex(xsi,pr->Numero(),Contact::Type::SpherePlanR,-9);
					}
				}
				ComputeContactForce(ctl, tx, ty, tz, T, N, Fx, Fy, Fz);
			...
			case Contact::Type::BodyPlanR: //12
				...
				if(ctl->delta < 0){
					...
					// Statique
						...
						xsi = ba->GetElongationManager().GetElongation(pr->Numero(),Contact::Type::BodyPlanR,na,-9);
						computeContactForceStatic(ctl, xsi, N, T, tx, ty, tz, Vn, Vt, Vtx, Vty, Vtz, h, k, g0, dat.muS,dat.muD);
						ba->GetElongationManager().AddElongationMutex(xsi,pr->Numero(),Contact::Type::BodyPlanR,na,-9);
					}
				}
			...
			case Contact::Type::SphereCone: //3
				...
				// Statique
					...
					xsi = a->GetElongationManager().GetElongation(cne->Numero(),Contact::Type::SphereCone,-9);
					computeContactForceStatic(ctl, xsi, N, T, tx, ty, tz, Vn, Vt, Vtx, Vty, Vtz, h, k, g0, dat.muS,dat.muD);
					a->GetElongationManager().AddElongationMutex(xsi,cne->Numero(),Contact::Type::SphereCone,-9);
				}
			...
			case Contact::Type::BodyCone: //13
				...
				// Statique
					...
					xsi = ba->GetElongationManager().GetElongation(cne->Numero(),Contact::Type::BodyCone,na,-9);
					computeContactForceStatic(ctl, xsi, N, T, tx, ty, tz, Vn, Vt, Vtx, Vty, Vtz, h, k, g0, dat.muS,dat.muD);
					ba->GetElongationManager().AddElongationMutex(xsi,cne->Numero(),Contact::Type::BodyCone,na,-9);
				}
			...
			case Contact::Type::SphereElbow: //4
				...
				// Statique
					...
					xsi = a->GetElongationManager().GetElongation(elw->numero,Contact::Type::SphereElbow,-9);
					computeContactForceStatic(ctl, xsi, N, T, tx, ty, tz, Vn, Vt, Vtx, Vty, Vtz, h, k, g0, dat.muS,dat.muD);
					a->GetElongationManager().AddElongationMutex(xsi,elw->numero,Contact::Type::SphereElbow,-9);
				}
			...
			case Contact::Type::BodyElbow: //14
				...
				// Statique
					...
					xsi = ba->GetElongationManager().GetElongation(elw->numero,Contact::Type::BodyElbow,na,-9);
					computeContactForceStatic(ctl, xsi, N, T, tx, ty, tz, Vn, Vt, Vtx, Vty, Vtz, h, k, g0, dat.muS,dat.muD);
					ba->GetElongationManager().AddElongationMutex(xsi,elw->numero,Contact::Type::BodyElbow,na,-9);
				}
            ...
            case Contact::Type::SphereHollowBall:
				...
				if(ctl->delta <= 0){
					...
					// Statique
						...
						computeContactForceStatic(ctl, xsi, N, T, tx, ty, tz, Vn, Vt, Vtx, Vty, Vtz, h, k, g0, dat.muS,dat.muD);
						// Enregistrement du nouveau Xsi
						a->GetElongationManager().AddElongationMutex(xsi,b->Num(),Contact::Type::SphereHollowBall,-9);
						b->GetElongationManager().AddElongationMutex(xsi,a->Num(),Contact::Type::SphereHollowBall,-9);
					}
				}
				// Calcul de la force de contact
				ComputeContactForce(ctl, tx, ty, tz, T, N, Fx, Fy, Fz);
            ...
            case Contact::Type::BodyHollowBall: //6
				...
				if(ctl->delta <= 0){
					...
					// Statique
						...
						computeContactForceStatic(ctl, xsi, N, T, tx, ty, tz, Vn, Vt, Vtx, Vty, Vtz, h, k, g0, dat.muS,dat.muD);
						// Enregistrement du nouveau Xsi
						a->GetElongationManager().AddElongationMutex(xsi,bb->Num(),Contact::Type::BodyHollowBall,nb);
						bb->GetElongationManager().AddElongationMutex(xsi,a->Num(),Contact::Type::BodyHollowBall,nb,-9);
					}
				}
				...
				// Incrementation de la force et du moment sur chaque corps
				a->AddForceMutex(-Fx, -Fy, -Fz);
				a->AddMomemtumMutex(-(lby*Fz-lbz*Fy), -(lbz*Fx-lbx*Fz), -(lbx*Fy-lby*Fx));

				bb->AddForceMutex(Fx, Fy, Fz);
				bb->AddMomemtumMutex((lay*Fz-laz*Fy), (laz*Fx-lax*Fz), (lax*Fy-lay*Fx));
