...
#pragma omp parallel shared(cell, Ntp, name, isMonitoringActivated) firstprivate(ct, cellBounds, Nct)
    {
        ...
        printf("Thread %d/%d - cellbounds X : %d - %d [Step = %d]\n", nthrd, thrd_start, thread_cellBounds.StartX(),
...
int Evolution::EvolveMelt(std::vector<Sphere *> cell, int &Ntp, char *name, double vr, double delayVr,
                          const bool isMonitoringActivated) noexcept {
    ...
    do {
        ...
        // Position anticipation
        Move::moveContainer(solids->plans, solids->disks, solids->cones, solids->elbows, solids->configuration.TIME,
                            dt / 2, solids->spheres, solids->gravity);
        Move::moveSphere(solids->spheres, dt / 2);
        Move::moveBodies(solids->bodies, dt / 2, solids->spheres);
        ...
        ContactDetection::sphContainer(solids->spheres, solids->plans, solids->disks, solids->cones, solids->elbows,
                                       solids->hollowBalls, Nct, ct, cell, solidCells, solids->configuration.Rmax);
        ...
        // Record data
        if (solids->configuration.record) {
            if (fabs((solids->configuration.TIME - solids->configuration.t0) - Ntp * (solids->configuration.dts)) <
                solids->configuration.dt * 0.99 && (solids->configuration.TIME - solids->configuration.t0 > 0.)) {
                ...
                ReadWrite::writeOutContainer(name, Ntp, solids->plans, solids->disks, solids->cones, solids->elbows,
                                             solids->configuration.outMode);
                ReadWrite::writeOutSphere(name, Ntp, solids->spheres, solids->configuration.outMode);
                ReadWrite::writeOutBodies(name, Ntp, solids->bodies, solids->configuration.outMode);
                ReadWrite::writeOutHollowBall(name, Ntp, solids->hollowBalls);
